...
const Rocket = forwardRef(function Rocket({ speedRef, boostRef }, ref) {
  const { scene } = useGLTF(rocketModel)

  const bobGroup = useRef(null)
  ...
  useFrame((state, delta) => {
    ...
    const t = state.clock.elapsedTime
    bobGroup.current.position.y = Math.sin(t * 1.6) * 0.06
    bobGroup.current.rotation.z = Math.sin(t * 1.1) * 0.02
  })
