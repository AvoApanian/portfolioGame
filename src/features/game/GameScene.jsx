...
export default function GameScene({ input, onHudUpdate, onPlanetChange, exitOrbitRef }) {
  ...
  useFrame((_, delta) => {
    ...
    if (orbit.current.active) {
      ...
      if (orbit.current.timer > ORBIT_DURATION || forcedExit) {
        ...
        logger.info('Game', 'Orbit ended', { planet: PLANETS[orbit.current.index]?.id })
      } else {
    ...
    if (boosting && !wasBoosting.current) {
      ...
      logger.info('Game', 'Boost engaged')
    }
    if (!boosting && wasBoosting.current) {
      logger.info('Game', 'Boost released')
    }
    wasBoosting.current = boosting

    const targetSpeed = wantsBrake ? BRAKE_TARGET_SPEED : boosting ? BOOST_MAX_SPEED : CRUISE_SPEED
    speed.current = THREE.MathUtils.lerp(speed.current, targetSpeed, 1 - Math.pow(0.001, delta * SPEED_SMOOTHING))

    soundManager.updateThrust(Math.min(1, speed.current / CRUISE_SPEED / 2), boosting)

    rocketRef.current.position.x += speed.current * delta

    const correction = 1 - Math.pow(0.001, delta * LINE_CORRECTION_SPEED)
    rocketRef.current.position.z += (0 - rocketRef.current.position.z) * correction
    rocketRef.current.position.y += (0 - rocketRef.current.position.y) * correction

    angle.current = THREE.MathUtils.lerp(angle.current, 0, correction)
    ...
    PLANETS.forEach((planet, i) => {
      ...
      if (dist > ORBIT_RESET_RADIUS) {
        ...
        logger.info('Game', 'Orbit started', { planet: planet.id })
      }
    })
    ...
    if (hudTimer.current >= HUD_UPDATE_INTERVAL && onHudUpdate) {
      hudTimer.current = 0
      onHudUpdate({
        speedPct: Math.min(1, speed.current / BOOST_MAX_SPEED),
        boostPct: boostFuel.current / 100,
        boosting,
  ...
  return (
    <>
      ...
      <Rocket ref={rocketRef} speedRef={speed} boostRef={boostAmount} />
    </>
  )
}
